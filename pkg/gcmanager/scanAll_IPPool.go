...
// Helps check if it is a valid static Pod (StatefulSet, KubeBlocks InstanceSet, or Kubevirt). If it is a valid static Pod, return true.
// Fork note (apecloud/spiderpool support/instanceset): the InstanceSet branch is the fork-specific extension
// to upstream PR #3778's helper. It is gated by EnableStatefulSet because InstanceSet replaces StatefulSet
// in KubeBlocks topology and shares the same "stable identity, GC must wait for controller intent" semantics.
func (s *SpiderGC) isValidStatefulsetOrKubevirt(ctx context.Context, logger *zap.Logger, podNS, podName, poolIP, ownerControllerType string) (bool, error) {
	...
	if s.gcConfig.EnableStatefulSet && ownerControllerType == constant.KindInstanceSet {
		isValidItsPod, err := s.itsMgr.IsValidInstanceSetPod(ctx, podNS, podName, constant.KindInstanceSet)
		if err != nil {
			logger.Sugar().Errorf("failed to check if InstanceSet pod IP '%s' should be cleaned or not, error: %v", poolIP, err)
			return true, err
		}
		if isValidItsPod {
			logger.Sugar().Warnf("no need to release IP '%s' for InstanceSet pod", poolIP)
			return true, nil
		}
	}
